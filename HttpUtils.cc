...
bool is_path_safe(const string &root_dir, const string &test_file) {
  ...
  char* ptr = realpath(const_cast<char*>(test_file.c_str()), actual);
 
  // test file is smaller than root path 
  // test file should contain root path (compare length)
  // check test file has a / between root and test file
  // check if test file is a valid string
  
  string testPath(ptr);
  uint16_t found = testPath.find(root_dir);
  uint16_t rootSize = root_dir.size();
  
  if(ptr == nullptr || found == std::string::npos || found > testPath.size()) {
    return false;
  }
  
  if (testPath.size() < rootSize) {
    std::cout << "test_file is greater than root" << std::endl;
    return false;
  } else {
    if (testPath.at(found +rootSize) != '/') {
      std::cout << "root and test file is not separated by /" << std::endl;
      return false;
    }
  }
...
string escape_html(const string &from) {
  ...
  for (; idx < ret.length(); idx++) {
    ...
    char ch = ret.at(idx);
    if (ch == '<') {
      ret.replace(idx, 1, "&lt;", 0, 4);
      idx += 3;
    } else if (ch == '>') {
      ret.replace(idx, 1, "&gt;", 0, 4);
      idx += 3;
    } else if (ch == '&') {
      ret.replace(idx, 1, "&amp;", 0, 5);
      idx += 4;
    } else if (ch == '\'') {
      ret.replace(idx, 1, "&apos;", 0, 6);
      idx += 5;
    } else if (ch == '\"') {
      ret.replace(idx, 1, "&quot;", 0, 6);
      idx += 5;
